...
def parse_stocksd_data(stock_symbol:str):
    ...
    stock_data = {
        ...
        for date_str, records in groupby(stock_data, key=lambda x: x.date_time.strftime("%Y%m%d"))
    }
    return stock_data
...
def handle_stocksd_chart(stock_symbol: str, range_values: tuple[int, int]):
    ...
    for date_str, records in stock_data.items():
        ...
        # 計算大股東持有率
        major_share_sum = sum(r.shares for r in records[range_values[0]-1: range_values[1]])
        major_share_ratio = (major_share_sum / key_record.shares) * 100 if key_record.shares else 0
