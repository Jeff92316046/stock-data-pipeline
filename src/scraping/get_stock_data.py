...
from repository.stock_share_distribution_repository import upsert_stock_share_distributions
import threading
...
def fetch_stockSD_data_by_symbol(stock:Stocks):
    with get_driver(debug=True) as driver:
        try:
            ...
            last_updated_at = stock.last_updated_at.strftime("%Y%m%d") if stock.last_updated_at else ""
            newer_dates = [option.get_attribute("value") for option in options if option.get_attribute("value") > last_updated_at]
            driver.find_element(
                XPATH,
                "//*[@id='StockNo']",
            ...
            for date in newer_dates:
                ...
                time.sleep(0.3)
                driver.find_element(XPATH,"//*[@id='form1']/table/tbody/tr[4]/td/input").click()
                table = driver.find_element(XPATH, "//*[@id='body']/div/main/div[6]/div/table/tbody")
...
def update_all_stockSD_data():
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_TASKS) as executor:
        future_to_code = {executor.submit(fetch_stockSD_data_by_symbol, stock): stock for stock in stock_list}
